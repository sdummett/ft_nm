...
int print_symtab_entries_x32(void *f, Elf32_Shdr * symtab_hdr, char *opts) {
	...
	Elf32_Sym_Nm *tmp = NULL;
	for (uint32_t i = 1; i < symtab_entries_num; i++) {
		Elf32_Sym *symbol = get_symbol_x32(f, symtab_hdr, i);

		if (head == NULL) {
			head = get_symbol_infos_x32(f, symtab_hdr, symbol);
			tmp = head;
		}
		else {
			tmp->next = get_symbol_infos_x32(f, symtab_hdr, symbol);
			tmp = tmp->next;
		}
...
static void reverse_linked_list(Elf32_Sym_Nm** head) {
    Elf32_Sym_Nm *prev = NULL;
    Elf32_Sym_Nm *current = *head;
    Elf32_Sym_Nm *next = NULL;

    while (current != NULL) {
        next = current->next;  // Store the next node

        // The real nm don't reverse same symbol name, we do the same
        if (next != NULL && stringcmp(current->symbol_name, next->symbol_name) == 0) {
...
static Elf32_Sym_Nm* merge(Elf32_Sym_Nm *list1, Elf32_Sym_Nm *list2) {
	if (list1 == NULL)
		return list2;
	if (list2 == NULL)
		return list1;

	Elf32_Sym_Nm *merged_list = NULL;
...
static void split(Elf32_Sym_Nm *head, Elf32_Sym_Nm **firstHalf, Elf32_Sym_Nm **secondHalf) {
	...
	Elf32_Sym_Nm *slow = head;
	Elf32_Sym_Nm *fast = head->next;
...
static void merge_sort(Elf32_Sym_Nm **head) {
	...
	Elf32_Sym_Nm *firstHalf;
	Elf32_Sym_Nm *secondHalf;
